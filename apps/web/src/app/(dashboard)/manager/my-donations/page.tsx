...
function getStatusBadge(status: string): { label: string; classes: string; icon: React.ReactNode } {
  switch (status) {
    case 'COMPLETED':
      return {
        label: 'שולם',
...
function PaymentStatusBanner({ data }: { data: PaymentStatusData }) {
  const monthName = (() => {
    ...
    return format(new Date(parseInt(year), parseInt(month) - 1, 1), 'MMMM yyyy', { locale: he });
  })();
...
export default function ManagerMyDonationsPage() {
