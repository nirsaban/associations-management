...
export default function OrganizationSetupWizard() {
  ...
  const handleComplete = async () => {
    ...
    try {
      // Step 1: Save org details and mark setup as complete
      await api.patch(API_ROUTES.ORGANIZATION.SETUP, {
        name: wizardData.name,
        slug: wizardData.slug,
        contactEmail: wizardData.contactEmail || undefined,
        contactPhone: wizardData.contactPhone || undefined,
        address: wizardData.address || undefined,
        monthlyPaymentAmount: wizardData.monthlyPaymentAmount || undefined,
        paymentInstructions: wizardData.paymentInstructions || undefined,
        setupCompleted: true,
      });
      ...
      if (wizardData.logoFile) {
        ...
        await api.post(API_ROUTES.ORGANIZATION.LOGO, formData, {
      ...
      setError(err.response?.data?.message || 'שגיאה בשמירת הנתונים. אנא נסה שוב.');
    } finally {
  ...
  const handleStartUsing = () => {
    // Redirect to admin dashboard after setup completes
